<template>
  <div>
    ...
    <div class="table-title">전체 상품 {{this.getGeneral.rows.length}} 개, 판매 중 상품 {{this.getGeneral.rows.length - this.getGeneralProgressing.rows.length}} 개, 입금 대기 상품 {{this.getGeneralProgressing.rows.length}}개</div>
    <div class="subcontents-define">전체 상품 {{this.getGeneral.rows.length}} 개
      <div class="add-button-container"><button class="crud-button" @click="showModal = 1">상품 추가 등록</button></div>
    </div>
    <vue-good-table
      :columns="getGeneral.columns"
      :rows="getGeneral.rows"
      :sort-options="{
        enabled: true,
    ...
    <div class="subcontents-define">입금 대기 상품 {{this.getGeneralProgressing.rows.length}} 개
      <div class="add-button-container"><button class="crud-button">입금 확인</button></div>
    </div>
    <vue-good-table
      :columns="getGeneralProgressing.columns"
      :rows="getGeneralProgressing.rows"
      :sort-options="{
        enabled: true,
        initialSortBy: {field: 'Code', type: 'asc'}
      }"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['getGeneralProgressing']),
